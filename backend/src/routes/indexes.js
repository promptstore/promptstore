...
export default ({ app, auth, constants, logger, services }) => {
  ...
  const { graphStoreService, indexesService, llmService, modelsService, vectorStoreService } = services;
  ...
  async function deletePhysicalIndexAndData(indexId) {
    const { name, nodeLabel, vectorStoreProvider, graphStoreProvider } = await indexesService.getIndex(
      indexId
    );
