...
import { OpenAIMessage, OpenAIChatCompletionResponse } from '../models/openai_types';
import { ParserService } from '../outputprocessing/OutputProcessingPipeline_types';
...
export enum MessageRole {
  system = 'system',
  developer = 'developer',  // With o1 models and newer, `developer` messages replace the previous `system` messages.
  user = 'user',
  assistant = 'assistant',
  function = 'function',
  tool = 'tool',
...
export interface Message<T = ContentType> {
  ...
  content: T;
  name?: string;
  refusal?: string | null;  // The refusal message generated by the model, if any.
  function_call?: FunctionCall;
  tool_calls?: ToolCall[];
  citation_metadata?: CitationMetadata;
  logprobs?: LogProbs;  // Log probability information for the choice.
  final?: boolean;
}
...
interface JsonSchemaFormat {
  name: string;  // The name of the response format.
  ...
  schema?: Schema;  // The schema for the response format, described as a JSON Schema object.
  strict?: boolean;  // Whether to enable strict schema adherence when generating the output.
}

interface ResponseFormat {
  type: string;  // One of 'text', 'json_object', or 'json_schema'. Defaults to 'text'.
  json_schema?: JsonSchemaFormat;  // Required when type is 'json_schema'. Enables Structured Outputs.
}

export interface ModelParams {
  max_tokens: number; // DEPRECATED in favor of max_completion_tokens, and not compatible with o1-series/newer models. The maximum number of tokens to generate in the chat completion. The total length of input tokens and generated tokens is limited by the model's context length. Defaults to inf.
  max_completion_tokens: number; // An upper bound for the number of tokens that can be generated for a completion, including visible output tokens and reasoning tokens. Replaces max_tokens for o1-series/newer models.
  reasoning_effort: string; // Constrains effort on reasoning for reasoning models. One of 'minimal', 'low', 'medium', or 'high'. Reducing reasoning effort can result in faster responses and fewer tokens used on reasoning.
...
export interface ChatRequest {
  ...
  tool_choice?: string | object; // Controls which (if any) tool is called by the model. 'none', 'auto', 'required', or a named tool { type: 'function', function: { name } }.
  parallel_tool_calls?: boolean; // Whether to enable parallel function calling during tool use. Defaults to true.
  best_of?: number;
  stream?: boolean;
  stream_options?: { include_usage?: boolean }; // Options for streaming response. Set { include_usage: true } to receive a final usage chunk while streaming.
  user?: string;
...
export interface PromptTokensDetails {
  cached_tokens?: number; // Cached tokens present in the prompt.
  audio_tokens?: number; // Audio input tokens present in the prompt.
}
...
export interface CompletionTokensDetails {
  ...
  rejected_prediction_tokens?: number; // When using Predicted Outputs, the number of tokens in the prediction that did not appear in the completion.
}
...
export interface ChatResponse {
  ...
  n: number;
  system_fingerprint?: string; // The backend configuration that the model runs with. Can be used with the seed request parameter to understand when backend changes have been made that might impact determinism.
...
export async function fromOpenAIChatResponse(
  ...
  const { id, created, model, usage } = response;
...
export function getText(messages: Message[]) {
  return messages.map(m => convertContentTypeToString(m.content)).join(PARA_DELIM);
}
